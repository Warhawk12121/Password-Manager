...
def Save():
    ...
    if password == "" or name == "" or uname == "":
        ...
        password_dict = {"Site": name, "Username": uname, "Password": password}
...
def Create_fun():
    ...
    Label(newWindow, text="Create a new password").grid(row=0, column=1, pady=10)
    ...
    pasword_entry = Entry(newWindow, textvariable=password_text, state=DISABLED).grid(row=3,
                                                                                      column=2)

    Button(newWindow, text="Generate a password ", command=generate, fg="#00FF00",
           bg="black").grid(row=4, column=1, pady=10)
    Button(newWindow, text="Save", command=Save, fg="#00FF00", bg="black").grid(row=5,
                                                                                column=1,
                                                                                pady=10)
...
def Save_fun():
    ...
    newWindow.geometry("600x400")
    newWindow.config(background="#F7672D")
    Label(newWindow, text="Store your passwords").grid(row=0, column=1, pady=10)
    ...
    Button(newWindow, text="Save", command=Save, fg="#00FF00", bg="black", pady=10).grid(row=4,
                                                                                         column=1)
...
#inserting the website's name into the ListBox
    ...
    pasword_entry = Entry(newWindow, textvariable=password_text, width=20,
                          state=DISABLED).grid(row=3, column=2)

    Button(newWindow, text="Show", command=Show, fg="#00FF00", bg="black", pady=10).grid(row=4,
                                                                                         column=1,
                                                                                         pady=10)
    Button(newWindow, text="Copy Pasword", command=copy_button, fg="#00FF00", bg="black",
           pady=10).grid(row=5, column=1, pady=10)


#Stuff for Main Window
Label(window, text="PASSWORD MANAGER", font=(20)).pack(pady=10)

create = Button(window, text="Create", command=Create_fun, fg="#00FF00",
                bg="black")  #remove parenthis of function after placing it
create.pack(pady=10)
